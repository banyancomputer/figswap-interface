...
// Note (amiller68) - LimitOrders Rely on this, so we can't remove it
// taking out the main logic for now, but this neeeds to be reimplemented
const useBentoMasterApproveCallback = (
  ...
  const approvalState = BentoApprovalState.UNKNOWN
  const approve = () => Promise.resolve()
  const getPermit = () => Promise.resolve(undefined)
