...
// Note (amiller68): #MetasmaskOnly - I deprecated these wallet options, but use this tag to find them later and reimplement them
// import { GamestopConnector } from './gamestop-connector'
...
export const getNetworkConnector = (): NetworkConnector => {
  ...
  return (network = new NetworkConnector({
    defaultChainId: defaultChainId ? Number(defaultChainId) : config.defaultChainId,
    urls: RPC,
  }))
}
...
// Note (al): We will only support MetaMask until other Connectors ship FilSnap support
export const SUPPORTED_WALLETS: { [key: string]: WalletInfo } = {
  // Note (al): I think Injected is the same as MetaMask
  // INJECTED: {
  //   connector: injected,
  //   name: 'Injected',
